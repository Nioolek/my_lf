...
# 图表样式 - 包含中文的架构图/流程图使用 MSYH
s_diagram = ParagraphStyle("DiagramCN", parent=styles["Code"],
    fontName="MSYH", fontSize=7.5, leading=11, textColor=HexColor("#1e293b"),
    backColor=HexColor("#f1f5f9"), leftIndent=3*mm, rightIndent=3*mm,
    spaceBefore=1*mm, spaceAfter=2*mm)
...
def Code(text): return Paragraph(_escape(text), s_code)
def Diagram(text): return Paragraph(_escape(text), s_diagram)
def AutoCode(text): return Paragraph(_escape(text), s_diagram if _has_chinese(text) else s_code)
